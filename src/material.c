...
lambertian_t lambertian_create_from_color(color_t albedo) {
  ...
  *texture = solid_texture_create(albedo);
  return lambertian_create((texture_t*)texture);
}

lambertian_t lambertian_create(texture_t* texture) {
  lambertian_t result = {.base = {MATERIAL_LAMBERTIAN}, .texture = texture};
  return result;
}
...
bool lambertian_scatter(lambertian_t* material, ray_t* ray, hit_record_t* hit_record, color_t* attenuation,
  ...
  *attenuation = texture_value(material->texture, hit_record->u, hit_record->v, &hit_record->point);
  return true;
}
...
bool metal_scatter(metal_t* material, ray_t* ray, hit_record_t* hit_record, color_t* attenuation,
                   ray_t* scattered_ray) {
...
dielectric_t dielectric_create(double refraction_index) {
  return (dielectric_t){{MATERIAL_DIELECTRIC}, refraction_index};
}
...
bool dielectric_scatter(dielectric_t* material, ray_t* ray, hit_record_t* hit_record, color_t* attenuation,
  ...
  *attenuation = col_create(1, 1, 1);
  double refraction_ratio =
      (hit_record->front_facing) ? (1.0 / material->refraction_index) : material->refraction_index;
